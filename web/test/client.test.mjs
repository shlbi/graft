...
async function harness() {
  ...
  const fetchImpl = async (url, options) => {
    ...
    if (url === '/api/demo') return { ok: true, json: async () => demoRun() };
...
test('actual client renders the sample, gates patch download and exports matching bytes', async () => {
  ...
  assert.match(ids['result-mode'].textContent, /AUTHORED SAMPLE/); assert.equal(ids['change-list'].children.length, 2);
